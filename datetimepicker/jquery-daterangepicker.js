    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        function init_datepicker() {
            ...
            box.find('.apply-btn').click(function() {
                ...
                var d1 = opt.start;
                var d2 = opt.end;
                if( d2 && !d1 ) opt.start = opt.end;
                if( d1 && !d2 ) opt.end = opt.start;
                var dateRange = opt.start == opt.end ? getDateString(new Date(opt.start)):getDateString(new Date(opt.start)) + opt.separator + getDateString(new Date(opt.end));
            ...
            box.find('.cancle-btn').click(function(){
            	closeDatePicker();
            	resetTodefault();
            })

			function resetTodefault(){
				var defaultVal = $(self).data('datepicker-default-value');
				opt.setValue.call(selfDom, defaultVal);
			}
        ...
        function checkAndSetDefaultValue() {
			...
			if(defaults.length == 1 && !opt.singleDate){
                defaults.push(defaults[0]);
            }
			if(!$(self).data('datepicker-default-value')){
				$(self).data('datepicker-default-value',__default_string);
			}
        ...
        function showSelectedInfo(forceValid, silent) {
            ...
            if (opt.start) {
            	...
            	if(!opt.end){
	                box.find('.end-day').html(getDateString(new Date(parseInt(opt.start))));
            	}
            }
            if (opt.end) {
            ...
            opt.setValue.call(selfDom, getDateString(new Date(opt.start)), getDateString(new Date(opt.start)), getDateString(new Date(opt.end)));
        ...
        function createDom() {
            ...
            html += '<div class="month-wrapper">' + '<table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="' + _colspan + '" class="month-name"></th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '<span class="next">&gt;</span>' : '') + '</th></tr><tr class="week-name">' + getWeekHead() + '</thead><tbody></tbody></table>';

            if (hasMonth2()) {
                html += '<div class="gap">' + getGapHTML() + '</div>' + '<table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;">' + (!opt.stickyMonths ? '<span class="prev">&lt;</span>' : '') + '</th><th colspan="' + _colspan + '" class="month-name"></th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">' + getWeekHead() + '</thead><tbody></tbody></table>'
            }
            //+'</div>'
			...
			if (opt.showTopbar) {
                ...
                html += '<div class="date-controls">'
                html += '<input type="button" class="apply-btn disabled' + getBtnClass('applyBtn') + '" value="' + lang('apply') + '" />';
                html += '<input type="button" class="cancle-btn disabled' + getBtnClass('cancleBtn') + '" value="' + lang('cancle') + '" />';
                html += '</div>';
                html += '</div>'
            }
        ...
        function getBtnClass(btnName) {
            ...
            if (opt[btnName+'Class'] !== '') {
                klass += ' ' + opt[btnName+'Class'];
            }
            return klass;
        }
