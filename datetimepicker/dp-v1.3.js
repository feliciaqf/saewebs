...
(function(){
	...
	function Cd(J,K){
	 	...
	 	var $cfg = J,
	        ...
	        $OPERA = $cfg.opera,
    		$cMark = false,
    		$dt,$tdt,$sdt,$lastInput,$ny,$c,$d,$dp;
	    
	    if($cfg.eCont){
	    	$dp = {};
	    	for (var p in K) if (typeof K[p] == "object") {
		        $dp[p] = {};
		        for (var pp in K[p]) $dp[p][pp] = K[p][pp];
		    } else $dp[p] = K[p];
	    }else $dp = K;
	    ...
	    function Picker() {
	        ...
	        $d.className = "picker-wrapper";
	        $d.innerHTML = '<div id=dpTitle><div class="navImg NavImgll"><a></a></div><div class="navImg NavImgl"><a></a></div><div style="float:left"><div class="menuSel MMenu"></div><input class=yminput></div><div style="float:left"><div class="menuSel YMenu"></div><input class=yminput></div><div class="navImg NavImgrr"><a></a></div><div class="navImg NavImgr"><a></a></div><div style="float:right"></div></div><div style="position:absolute;overflow:hidden"></div><div></div><div id=dpTime><div class="menuSel hhMenu"></div><div class="menuSel mmMenu"></div><div class="menuSel ssMenu"></div></div><div id=dpQS></div><div id=dpControl><input class=dpButton id=dpClearInput type=button><input class=dpButton id=dpTodayInput type=button><input class=dpButton id=dpOkInput type=button></div>';
	    ...
	    Picker.prototype = {
	        init: function() {
	            ...
	            this.autoPickDate = $dp.autoPickDate == null ? $dp.isShowOK ? false : true : $dp.autoPickDate;
	                ...
	                for (D = 0; D < 7; D++) {
	                    ...
	                    if (K) {
	                        if (this.checkValid(_, "d", D)) {
	                            ...
	                            H.a('data-role="dayMenu" data-year="'+ _.y+'" data-month="'+ _.M+'" data-day="'+ _.d+'" data-isr="'+ L +'" ');
	        ...
	        _ft:function(){
				var E = new sb();
				E.a('<table cellspacing=0 cellpadding=0 border=0><tr><td rowspan=2><span class=dpTimeStr>');
				E.a($lang.timeStr);
				E.a('</span>&nbsp;<input class=tB maxlength=2><input value=":" class=tm readonly><input class=tE maxlength=2><input value=":" class=tm readonly><input class=tE maxlength=2></td><td class="wdatef0"><button class=dpTimeUp></button></td></tr><tr><td class="wdatef0"><button class=dpTimeDown></button></td></tr></table>');
				return E.j();
	        },
	        ...
	        _fHMS: function(A, $,isR) {
	            var B = $dp.hmsMenuCfg[A],
	                C = B[0],
	                _ = B[1];
	            $d[A + "D"].innerHTML = this._f(A, $ - 1, _, Math.ceil($ / C / _), "i*" + _ + "*" + C + "+j*" + C,isR);
	            this.initEvents("td",$d[A + "D"]);
	        },
	        _fH: function() {
	        	var isR = typeof arguments[arguments.length-1] == "boolean"?arguments[arguments.length-1]:false;
        		this._fHMS("H", 24,isR);
	        },
	        _fm: function() {
	        	var isR = typeof arguments[arguments.length-1] == "boolean"?arguments[arguments.length-1]:false;
	        ...
	        _fillQS: function(C, A) {
	            this.initQS();
	            var $ = A ? ["tceles kciuq rekci", "PetaD",].join("").split("").reverse().join("") : $lang.quickStr,
	                B = this.QS,
	                E = B.style,
	                _ = new sb();
	        ...
	        draw: function() {
	            ...
	            if ($dp.doubleCalendar) {
	                $c.autoPickDate = $c.autoPickDate == null? $c.isShowOK?false: true :$c.autoPickDate;
	                $dp.isShowOthers = false;
	                $d.className = "picker-wrapper picker-wrapper2";
	                var $ = new sb();
	                ...
	                $.a("</td></tr></table>");
	                $d.dDiv.innerHTML = $.j();
	                if($dp.has.st && $dp.doubleValue){
	                	if($d.tDiv.childNodes.length>3){
	                		$d.tDiv.removeChild($d.tDiv.childNodes[3]);
	                	}
	                	
	                	if($d.tDiv.className.indexOf("dobuleTime") == -1) $d.tDiv.className = "dobuleTime";
		                $ = new sb();
		                ...
		                $.a("</td></tr></table>");
		                tmpDiv.innerHTML = $.j();
		                $d.tDiv.appendChild(tmpDiv.firstChild);
		                var ipts = gByN("input",$d), btns = gByN("button",$d);
		                $d.HI = ipts[4];
			            $d.mI = ipts[6];
			            $d.sI = ipts[8];
			            $d.rHI = ipts[9];
	            		...
	            		$d.downButton = btns[1];
	            		$d.rupButton = btns[2];
	            		$d.rdownButton = btns[3];
	            		_inputBindEvent("H,m,s,rH,rm,rs");
	                }
	                if($dp.doubleValue){
	                	if($d.rgDiv){
	                		$d.removeChild($d.rgDiv);
	                	}
	                	$ = new sb();
	                	$.a('<div class="drp_top-bar normal"><div class="normal-top"><span>');
	                	$.a($lang.rangStr);
	                	$.a('</span> <input value="" class="rgipt start-range" readonly=""> <span class="separator-day"> - </span> <input value="" class="rgipt end-range" readonly=""></div></div>');
	                	tmpDiv.innerHTML = $.j();
	                	$d.insertBefore(tmpDiv.firstChild,$d.qsDiv);
	                	var divs = gByN("div",$d),ipts;
	                	$d.rgDiv = divs[16];
	                	ipts = gByN("input",$d.rgDiv);
	                	$d.rgS = ipts[0];
	                	$d.rgE = ipts[1];
	                	_inputSetValue("rgS,rgE");
	                ...
	                tmpDiv.innerHTML = this._ft();
	                $d.tDiv.appendChild(tmpDiv.firstChild);
	                var divs = gByN("div") , ipts = gByN("input"), btns = gByN("button"), spans = gByN("span");
	                $d.HI = ipts[2];
		            $d.mI = ipts[4];
		            $d.sI = ipts[6];
		            $d.upButton = btns[0];
            		$d.downButton = btns[1];
		            _inputBindEvent("H,m,s");
	            }
	            tmpDiv = null;
	            		...
	            		case "dayMenu":
	                		obj.onclick = function(){
	                			var _y = parseInt(this.getAttribute("data-year")),
			            			...
			            			isR = !!parseInt(this.getAttribute("data-isr")),
			            			a = [];
			            		...
			            		if(_y && _M && _d){
			            			...
			            			if($dp.doubleCalendar && isR){
			            				a.push(isR);
			            			}
			            			day_Click.apply(null,a);
				            		
				            		var dms = $d.dDiv.getElementsByTagName("td");
				            		for(var j=0,len = dms.length;j<len;j++){
				            			if(dms[j].className.indexOf("Wselday") >=0){
				            				dms[j].className = dms[j].className.replace("Wselday","");
				            				break;
				            			}
				            		}
				            		if(this.className.indexOf("Wselday") == -1) this.className = "Wselday";
				            		if($dp.doubleCalendar && isR){
				            			if(this.className.indexOf("start-day") == -1) this.className = "start-day";
				            			if(this.className.indexOf("end-day") == -1) this.className = "end-day";
				            		}
	        ...
	        initBtn: function() {
		        ...
		        if($dp.has.st && $dp.doubleCalendar && $dp.doubleValue){
			        $d.rupButton.onclick = function() {
			            updownEvent(1);
			        };
			        $d.rdownButton.onclick = function() {
			            updownEvent(-1);
			        };
		        }
	    ...
	    function c(_, A) {
	        var $ = A,
	        	_r = _.replace("r","");
	        if (_r == "M") $ = makeInRange(A, 1, 12);
	        else if (_r == "H") $ = makeInRange(A, 0, 23);
	        else if ("ms".indexOf(_r) >= 0) $ = makeInRange(A, 0, 59);
	        if (A == $ + 1) $ = $sdt[_];
	        if ($sdt[_r] != $ && !callFunc(_r + "changing")) {
	            var B = $c.checkRange();
	            if (B == 0) sv(_, $);
	            else if (B < 0) _setAll($c.minDate);
	            else if (B > 0) _setAll($c.maxDate);
	            $d.okI.disabled = !$c.checkValid($sdt);
	            if ("yMd".indexOf(_r) >= 0) $c.draw();
	            callFunc(_r + "changed");
	        }
	    }
	    ...
	    function day_Click(F, B, _, D, C, A) {
	        var $ = new DPDate($dt.y, $dt.M, $dt.d, $dt.H, $dt.m, $dt.s),
	        	isR = typeof(arguments[arguments.length-1]) == "boolean"?arguments[arguments.length-1]:undefined;
	        $dt.loadDate(F, B, _, D, C, A);
	        if (!callFunc("onpicking")) {
	            var E = $.y == F && $.M == B && $.d == _;
	            if (!E && arguments.length != 0 && !isR) {
	                c("y", F);
	                c("M", B);
	                c("d", _);
	                $c.currFocus = $dp.el;
	    ...
	    function sv(_, $) {
	        ...
	        if ($ == null) $ = $dt[_];
	        $sdt[_] = $dt[_] = $;
	        if ("yHms".indexOf(_r) >= 0) $d[_ + "I"].value = $;
	        if (_r == "M") {
	            $d[_+"I"]["realValue"] = $;
	            $d[_+"I"].value = $lang.aMonStr[$ - 1];
	        }
	    }
	    ...
	    function updownEvent(_) {
	        var A = $c.currFocus,
	            $ = $dp.hmsMenuCfg;
	        if (A != $d.HI && A != $d.mI && A != $d.sI && A != $d.rHI && A != $d.rmI && A != $d.rsI) A = $d.HI;
	        switch (A) {
	            case $d.HI:
	                c("H", $dt.H + _ * $.H[0]);
	                break;
	    ...
	    function _foundInput($) {
	        var A, B, _ = "y,M,H,m,s,ry,rM,rH,rm,rs".split(",");
	    ...
	    function _focus($) {
	        ...
	        showB(_);
	        if ("Hms".indexOf(A) >= 0) {
	            _.style.marginLeft = Math.min(this.offsetLeft, $d[(isR?"rsI":"sI")].offsetLeft + 60 - _.offsetWidth) + "px";
	            _.style.marginTop = this.offsetTop - _.offsetHeight - 2 +"px";
	        }
	    }
		...
		function _inputSetValue($) {
	        var A = $.split(",");
	        for (var B = 0; B < A.length; B++) {
	            var _ = A[B],C;
	            C = $c.getNewDateStr();
	            if(_ == "rgE"){
	            	C = new DPDate($dt.ey,$dt.eM,$dt.sd,$dt.rH,$dt.rm,$dt.rs);
	            }else if(_ == "rgS"){
	            	C = new DPDate($dt.sy,$dt.sM,$dt.sd,$dt.H,$dt.m,$dt.s);
	            }
	            C = $c.getNewDateStr(C);
	            $d[_].value = C;
	        }
