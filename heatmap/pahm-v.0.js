...
(function() {
	try {
		...
		var pl = document.getElementsByTagName('title')[0];
		pl.parentNode.insertBefore(l, pl);
				...
				send: function() {
					...
					if (dr.length) {
						for (i = 0, l = dr.length; i < l; i++) {
							...
							console && console && console.log("%csend... " + dr[i].data + "   pageNum : " + dr[i].pageNum, 'color:#0FB2C7');
							dr[i].data = {};
							h.data = [];
						}
					}
	...
	(function() {
		try {
            ...
            e.add(window,'load',function(){
               	var debug = u.getQueryString('debug');
                if(debug == "true"){
                    mobileConsole.show();
                } 
            })
